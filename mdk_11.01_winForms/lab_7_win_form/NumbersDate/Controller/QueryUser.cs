...
namespace NumbersDate.Controller
{
    class QueryUser
    {
        ...
        public QueryUser(string Conn)
        {
            connection = new OleDbConnection(Conn);
            bufferTable = new DataTable();
        }
        public void Compare(string Log, string Pas)
        {
            connection.Open();
            dataAdapter = new OleDbDataAdapter($"SELECT * FROM Преподаватель WHERE Log = {Log}, Pas={Pas}", connection);
